// functions/equip/index.ts —— 佩戴装备（token 归属校验 → 库内白名单 RPC）
// 部署：Supabase Dashboard → Edge Functions → equip → 粘贴 → Deploy
...
const EQUIP_KEYS = ["summit", "extremeWin", "fish6", "heliMan", "crocHit", "snowWalker", "thornWalker", "unique",
  "snowDaughter", "ironWall", "endlessEra", "faceIt", "kingTop"];   // 13 成就可佩戴（2026-08-12 删 fartSkill/runFast/ymYC）
...
Deno.serve(async (req) => {
  ...
  // ③ 库内 RPC：白名单校验（8 种装备 key）
  const { error } = await sb.rpc("update_equip", {
    p_nickname: nickname, p_equip: equip ?? null,
  });
