// functions/achieve/index.ts —— 成就同步（token 归属校验 → 库内白名单 RPC）
// 部署：Supabase Dashboard → Edge Functions → achieve → 粘贴 → Deploy
...
const ACH_KEYS = ["summit", "extremeWin", "fish6", "heliMan", "crocHit", "snowWalker", "thornWalker", "unique",
  "snowDaughter", "ironWall", "endlessEra", "faceIt", "kingTop"];   // 13 成就（2026-08-12 删 fartSkill/runFast/ymYC）
...
Deno.serve(async (req) => {
  ...
  const { nickname, token, achievements } = body;

  // ① 参数校验（8 key 白名单 + boolean 值）
  if (!isStr(nickname, 1, 10)) return json({ error: "invalid_request" }, 400);
  if (!isStr(token, 1, 64)) return json({ error: "invalid_request" }, 400);
  if (achievements === undefined || achievements === null || typeof achievements !== "object" || Array.isArray(achievements))
    return json({ error: "invalid_request" }, 400);
  for (const k of Object.keys(achievements)) {
    if (!ACH_KEYS.includes(k)) return json({ error: "invalid_request" }, 400);
    if (typeof achievements[k] !== "boolean") return json({ error: "invalid_request" }, 400);
  }

  // ② token 归属校验（防冒名给他人刷成就）
  ...
  // ③ 库内 RPC：白名单校验
  const { error } = await sb.rpc("sync_achievements", {
    p_nickname: nickname, p_ach: achievements ?? {},
  });
